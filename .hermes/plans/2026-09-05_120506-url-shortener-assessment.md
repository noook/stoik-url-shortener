...
- **Icons: `unplugin-icons` (Lucide set) — adopted for app-level icon usage.** Auto-imported, tree-shaken per-icon components (`import LogOutIcon from '~icons/lucide/log-out'`), no icon font or manual sprite management. `lucide-react` itself stays only as a transitive dependency shadcn's own generated components use internally (e.g. `Select`'s chevron) — not swapped out, since that's shadcn's own code, not app code.
...
4. **Rate limiting** — not part of this build. Flagged by the user as a good candidate topic to raise or explore in a follow-up interview round (e.g. per-token or per-IP throttling on the redirect and auth endpoints) rather than something to implement now.
